...
class ModernizeSwarmCoordinator:
    ...
    def execute_swarm_turn(self, query: str, repo_path: str = "data/insurance_claims_app") -> Dict[str, Any]:
        ...
        if any(w in q_lower for w in ["ingest", "scan", "discover", "upload", "catalog", "directory"]):
            ...
            w in q_lower for w in ["impact", "break", "blast", "change", "affect", "touch", "caller", "dependency"]
        ):
            target_agent = "impact_analysis_agent"
            frontman_reasoning += (
                "Classified as Change Impact & Blast-Radius Assessment. Delegating to Impact Analysis Agent."
            )
        elif any(
            w in q_lower for w in ["rule", "business rule", "validate", "eligibility", "threshold", "grace", "formula"]
        ):
            target_agent = "business_rules_agent"
            frontman_reasoning += (
                "Classified as Business Logic Formalization & Verification inquiry. "
                "Delegating to Business Rules Agent."
            )
        elif any(
            w in q_lower
            for w in ["moderniz", "cloud", "6r", "roadmap", "readiness", "score", "microservice", "decouple", "domain"]
        ):
            target_agent = "modernization_advisor_agent"
            frontman_reasoning += (
                "Classified as Cloud Modernization & 6R Roadmap request. Delegating to Modernization Advisor Agent."
            )
        else:
            target_agent = "knowledge_graph_agent"
            frontman_reasoning += (
                "Classified as Architecture & Knowledge Graph Fabric inquiry. Delegating to Knowledge Graph Agent."
            )
        ...
        if target_agent == "discovery_agent":
            ...
            disc_thought = (
                f"Scanning repository at '{repo_path}'. Cataloging files into code, database DDLs, and specs."
            )
            trace.append(
                {
                    "step": 2,
                    ...
                    "thought": disc_thought,
                    "reasoning": disc_thought,
            ...
            trace.append(
                {
                    ...
                    "thought": (
                        f"Identified change seed '{target_entity}'. "
                        "Requesting bidirectional BFS blast-radius calculation from Knowledge Graph Agent."
                    ),
                    "reasoning": (
                        f"Identified change seed '{target_entity}'. "
                        "Requesting bidirectional BFS blast-radius calculation from Knowledge Graph Agent."
            ...
            trace.append(
                {
                    ...
                    "thought": (
                        f"Traversed graph for '{target_entity}'. "
                        f"Found {blast_res.get('upstream_count', 0)} upstream callers and "
                        f"{blast_res.get('downstream_count', 0)} downstream dependencies. "
                        f"Risk Level: {blast_res.get('risk_level')}."
                    ),
                    "reasoning": (
                        f"Traversed graph for '{target_entity}'. "
                        f"Found {blast_res.get('upstream_count', 0)} upstream callers and "
                    ...
                    "result_summary": (
                        f"Risk: {blast_res.get('risk_level')} ({blast_res.get('total_affected_count')} affected)"
                    ),
                }
            )
            ...
            trace.append(
                {
                    ...
                    "thought": (
                        f"Evaluating blast radius for '{target_entity}'. "
                        "Recommending isolation patterns based on coupling."
                    ),
                    "reasoning": (
                        f"Evaluating blast radius for '{target_entity}'. "
                        "Recommending isolation patterns based on coupling."
            ...
            trace.append(
                {
                    ...
                    "thought": (
                        f"Dynamically extracted {rules_res.get('total_rules_extracted', 0)} "
                        "formalized business rules from AST and stored procedures."
                    ),
                    "reasoning": (
                        f"Dynamically extracted {rules_res.get('total_rules_extracted', 0)} "
                        "formalized business rules from AST and stored procedures."
                    ...
                    "result_summary": (
                        f"Extracted {rules_res.get('total_rules_extracted', 0)} rules with verified source citations."
                    ),
                }
            )
            ...
            trace.append(
                {
                    ...
                    "thought": (
                        "Executed cross-artifact consistency check. "
                        f"Detected {val_res.get('total_discrepancies', 0)} contradictions/risks."
                    ),
                    "reasoning": (
                        "Executed cross-artifact consistency check. "
                        f"Detected {val_res.get('total_discrepancies', 0)} contradictions/risks."
            ...
            trace.append(
                {
                    ...
                    "thought": (
                        "Clustering graph topology into bounded contexts. "
                        f"Identified {len(comm_res.get('candidate_domains', []))} candidate microservice domains."
                    ),
                    "reasoning": (
                        "Clustering graph topology into bounded contexts. "
                        f"Identified {len(comm_res.get('candidate_domains', []))} candidate microservice domains."
            ...
            trace.append(
                {
                    ...
                    "thought": (
                        "Calculated composite readiness score: "
                        f"{tool_results['readiness'].get('overall_readiness_score')}/100 "
                        f"({tool_results['readiness'].get('grade')})."
                    ),
                    "reasoning": (
                        "Calculated composite readiness score: "
                        f"{tool_results['readiness'].get('overall_readiness_score')}/100 "
            ...
            trace.append(
                {
                    ...
                    "thought": (
                        "Ran Personalized PageRank and semantic retrieval. "
                        f"Fused top {len(rag_res.get('fused_evidence', []))} evidence items with RRF."
                    ),
                    "reasoning": (
                        "Ran Personalized PageRank and semantic retrieval. "
                        f"Fused top {len(rag_res.get('fused_evidence', []))} evidence items with RRF."
        ...
        if self._client:
            try:
                system_prompt = (
                    ...
                    "Synthesize a clear, authoritative, evidence-backed modernization answer based on "
                    "the query and tool results. Always include exact source line citations "
                    "(e.g. `PolicyValidationService.java:21-29`) and clear risk/architectural advice."
                )
                user_content = f"""
...
"""
        ...
        if "blast_radius" in tool_results:
            ...
            up_items = (
                ", ".join([f"`{i['node_id']}` ({i['edge_type']})" for i in br.get("upstream_impact", [])]) or "None"
            )
            down_items = (
                ", ".join([f"`{i['node_id']}` ({i['edge_type']})" for i in br.get("downstream_impact", [])]) or "None"
            )
            return (
                ...
                "An Anti-Corruption Layer (ACL) with Change Data Capture (CDC) is recommended "
                "to prevent breaking upstream callers."
            )
            ...
            rule_lines = "\n".join(
                [
                    ...
                    f"(`{r['source_file']}:{r['line_start']}-{r['line_end']}`): {r['specification']}"
                    for r in rules[:6]
                ]
            )
            disc_lines = "\n".join(
                [f"- ⚠️ **{d['title']}** ({d['severity']}): {d['description']}" for d in discrepancies[:3]]
            )
            return (
                f"### Dynamically Extracted Business Rules Catalog\n\n"
                f"The swarm extracted **{len(rules)} business rules** directly from code AST "
            ...
            strat_summary = "\n".join(
                [
                    f"- **{s['component']}** ({s['type']}): **{s['strategy_6r']}** -> "
                    f"{s['target_pattern']} ({s['priority']})"
                    for s in strats[:5]
                ]
            )
            return (
                f"### Modernization Readiness & 6R Strategy Assessment\n\n"
                f"- **Overall Modernization Readiness Score**: "
                f"**{rd.get('overall_readiness_score')}/100** ({rd.get('grade')})\n"
                ...
                f"{strat_summary}\n\n"
                "**Migration Phasing**: Refactor stateless business validation into serverless functions in "
                "**Phase 1**; replatform core adjudication services in **Phase 2**."
            )
            ...
            citations = "\n".join(
                [f"- **{e.get('label')}** (`{e.get('citation')}`): {e.get('content')}" for e in evidence[:4]]
            )
            return (
